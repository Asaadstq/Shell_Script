...
- The first line of every Bash script ir **shebang** `#!`:
...
1. Create an empty file named `hello.sh`.
```bash
touch hello.sh
```

2. Write the script
- Before adding content to the file, you must open it.
```bash
nano hello.sh
```
>[!Note]
> `nano` is a simple editor used to create and edit files directly in the command line.
...
> `./hello.sh` tells Ubuntu to run the file located here. Without `./` the system searches only in system directories
